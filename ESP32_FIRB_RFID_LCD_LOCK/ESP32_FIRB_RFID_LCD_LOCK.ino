...
void setup() {
  ...
  //   EEPROM.commit();
  // }
  if (EEPROM.read(1) == 1) {  // Haved Master Card
    Serial.println("I have Master Card !!");
    Serial.println("-----------------------------------");
  } else {  // Haven't Master Card
...
bool check_card(byte a[], byte b[]) {
  bool match;
  if (a[0] != NULL) {  // check card "a" empty ?
    match = true;
    for (int k = 0; k < 4; k++) {
      if (a[k] != b[k]) {
        match = false;
...
//-------------------------- CARD IS MASTER----------------//
bool is_master(byte a[]) {
  if (check_card(a, master_card) == true) {  // check card "a" is master card ?
    return true;
  } else {
    return false;
  }
...
bool find_id(byte id[]) {
  ...
  for (int i = 1; i <= count; i++) {
    ...
    if (check_card(id, store_card) == true) {  // compare saved card with "id"
      return true;
...
void write_id(byte a[]) {
  if (find_id(a) == false) {  // find "a" in EPPROM
...
int find_id_address(byte id[]) {
  ...
  for (int i = 1; i <= num_card; i++) {
    ...
    if (check_card(id, store_card) == true) {  // ktra card co trong bo nho ?
      return i;
      break;
    }
  }
...
void button() {
  if (lock_bt == 1) {
    ...
    if (digitalRead(bt_up) == 0) {  // choose YES
      yes = true;
    ...
    if (digitalRead(bt_down) == 0) {  // choose NO
      no = true;
      lock_add_delete = false;
      delay(50);
      while (digitalRead(bt_down) == 0) {
...
void main_once_option() {
  ...
  if (tt_choose == 0 && option_main == 0) {
    ...
    lcd.setCursor(16, 3);
    lcd.print("NO");
    if (lock_add_delete == false && yes == true) {  // CHOOSE YES
      lcd.clear();
      lcd.setCursor(0, 0);
    ...
    if (lock_add_delete == false && no == true) {  // CHOOSE NO
      lcd.clear();
      lcd.setCursor(0, 0);
      lcd.print("Return Main Menu");
      delay(3000);
